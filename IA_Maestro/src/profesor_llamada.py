...
def clean_for_speech(text: str) -> str:
    ...
    import re
    # Quita markdown básico (**negrita**, *cursiva*)
    text = re.sub(r'\*\*([^*]+)\*\*', r'\1', text)
    text = re.sub(r'\*([^*]+)\*', r'\1', text)
    # Quita bullets comunes (- * + • ● ▪ ‣ ◦) al inicio de línea
    text = re.sub(r'^\s*([+\-*•●▪‣◦]{1,3})\s+', '', text, flags=re.MULTILINE)
    # Quita enumeraciones tipo "1. ", "2) ", "a) ", "b. " al inicio
    text = re.sub(r'^\s*(?:[0-9]{1,3}|[a-zA-Z])([\.)\-])\s+', '', text, flags=re.MULTILINE)
    # Elimina símbolos sueltos + o - entre espacios que suelen leerse como "más" o "guión"
    text = re.sub(r'(?<=\s)[+\-](?=\s)', ' ', text)
    # Sustituye múltiples saltos de línea por punto y espacio
    text = re.sub(r'\n+', '. ', text)
    # Colapsa espacios múltiples
    text = re.sub(r'\s+', ' ', text)
    # Limpieza final de puntos repetidos
...
def make_client() -> Groq:
    load_dotenv()
    # Intenta primero la clave cifrada, luego la plana (compatibilidad)
    ...
    if api_key:
        try:
            ...
            print(f"Error al descifrar API key: {e}")
            sys.exit(1)
    else:
        api_key = os.getenv("GROQ_API_KEY")
    
    if not api_key:
        print("Error: Falta la variable de entorno GROQ_API_KEY o GROQ_API_KEY_ENCRYPTED.")
